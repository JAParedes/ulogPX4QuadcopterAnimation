...
% Also, shows autotuned RCAC theta parameters [1].
...
%Reading flight parameters (saved at start of flight)
...
% Reading autotuned RCAC theta parameters
rcac_att_theta_msg = params.Values(6:8);
rcac_rate_theta_msg = params.Values(16:24);
rcac_pos_theta_msg = params.Values(13:15);
rcac_vel_theta_msg = params.Values(25:33);
...
timeList = local_pos_TT_num.timestamp;
...
lenArr = length(1:dk:minL);

% The autotuned RCAC values don't change, this is just for visulization
% purposes.
rcac_att_theta = (rcac_att_theta_msg*ones(1,lenArr))';
rcac_rate_theta = (rcac_rate_theta_msg*ones(1,lenArr))';
rcac_pos_theta = (rcac_pos_theta_msg*ones(1,lenArr))';
rcac_vel_theta = (rcac_vel_theta_msg*ones(1,lenArr))';
...
for ii = 1:length(xx)
    ...
    % Plotting autotuned RCAC theta variables
    subplot(plots(2));
